...
export class Game {
  constructor(scene, camera, renderer, config = {}) {
    ...
    // Completion platform position/size (can be updated externally via debug UI)
    this.platformPos = new THREE.Vector3(-32.1, -6.73, 0.16);
    this.platformSize = new THREE.Vector3(3.55, 0.3, 3.55);

    // Checkpoints array populated externally; each: { pos, size, activated, wasIn }
    this.checkpoints = [];

    // Kill bricks array populated externally; each: { pos, size }
    this.killBricks = [];

    // Active respawn point saved when a checkpoint is touched
    this.activeCheckpoint = null;
    this.enterWasPressed = false;

    // Particle explosions for checkpoint activation
    this.checkpointExplosions = [];

    // Kill-brick hit cooldown (ms timestamp)
    this.lastKillBrickTime = 0;

    // Timer DOM cache — avoids querySelector + textContent write every frame
    this._timerEl = null;
    this._lastTimerText = '';

    // Per-frame animation callbacks registered by level setup
    this.levelAnimations = [];

    // Callback(index) fired when a checkpoint is first activated
  ...
  initTouchControls() {
    ...
    this.eventListeners.touchstartJump = (e) => {
      ...
      if (this.playerOnFloor) this.playerVelocity.y = 11;
  ...
  controls(deltaTime) {
    ...
    if (this.playerOnFloor && this.keyStates['Space']) this.playerVelocity.y = 16;
  }
  ...
  createCheckpointExplosion(position) {
    // Build a radial-gradient circle texture so particles are round, not square
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Uniform sphere distribution for equal spread in all directions
  ...
  animate() {
    ...
    // Enter: clear active checkpoint, reset all cp.activated, teleport to original spawn
    ...
    if (moving && this.playerOnFloor) {
      this.bobTime += frameDelta * 11;
      this.camera.position.y += Math.sin(this.bobTime) * 0.042;
      this.camera.rotation.z = Math.sin(this.bobTime * 0.5) * 0.013;
    } else {
      this.camera.rotation.z *= 0.78;
    }
    ...
    if (this.useBalls) {
      ...
      if (this.ballsThrown >= this.NUM_SPHERES) {
        if (cooldownFill) {
          ...
          cooldownFill.style.background = 'linear-gradient(90deg, #cc0000 0%, #ff2200 100%)';
          cooldownFill.style.boxShadow = 'inset 0 2px 4px rgba(255,255,255,0.1)';
        }
    ...
    // Level-specific per-frame animations (e.g. floating labels)
