...
export function loadLevel(scene, worldOctree, onProgress) {
  return new Promise((resolve, reject) => {
    ...
    gltfLoader.load('level-2.glb',
      ...
      (xhr) => { if (xhr.lengthComputable) { glbProgress = (xhr.loaded / xhr.total) * 0.999; reportProgress(); } },
      (error) => reject(error)
    );
    ...
    new RGBELoader().load(level2Config.hdrPath,
      ...
      (xhr) => { if (xhr.lengthComputable) { hdrProgress = (xhr.loaded / xhr.total) * 0.999; reportProgress(); } },
      (error) => reject(error)
    );
  });
}
